import curses

def page(stdscr):
    curses.start_color()
    curses.init_pair(1, curses.COLOR_WHITE, curses.COLOR_BLUE)

    while True:
        stdscr.clear()
        stdscr.bkgd(' ', curses.color_pair(1))
        
        height, width = stdscr.getmaxyx()

        title = "Установка flexOS"
        x_title = (width // 2) - (len(title) // 2)
        stdscr.addstr(1, x_title, title, curses.color_pair(1))

        stdscr.hline(2, 2, curses.ACS_HLINE, width - 4)

        control_text = "Укажите путь, куда будет установлен flexOS:"
        stdscr.addstr(4, 2, control_text, curses.color_pair(1))

        input_text = ""
        input_y = 6
        input_x = 2

        stdscr.addstr(input_y, input_x, input_text, curses.color_pair(1))
        stdscr.refresh()

        while True:
            key = stdscr.getch()

            if key == curses.KEY_BACKSPACE or key == 127:
                input_text = input_text[:-1]
            elif key in (curses.KEY_ENTER, 10):
                break
            elif key == curses.KEY_F3:
                exit()
            elif 32 <= key <= 126:
                input_text += chr(key)

            stdscr.addstr(input_y, input_x, ' ' * (width - input_x - 1))
            stdscr.addstr(input_y, input_x, input_text, curses.color_pair(1))
            stdscr.refresh()

        stdscr.hline(height - 3, 2, curses.ACS_HLINE, width - 4)

        keys_text = "F3: Выход  |  Enter: Продолжить"
        x_keys = (width // 2) - (len(keys_text) // 2)
        stdscr.addstr(height - 2, x_keys, keys_text, curses.color_pair(1))

        stdscr.refresh()